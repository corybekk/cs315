...
public:
  ...
  List* multiply(int x, int carry) {
    ...
    if(head == NULL) return new List(carry); 
    else{
      List* L1 = copy();
  ...
  void print() {
    ...
    while(head!= NULL){
      temp->insert(head->getKey()); // reverses the list
      head = head->getNext();
    }
    while(temp->head->getKey()== 0){ // ignores the leading zeros
      temp->head = temp->head->getNext();
    }
    while(temp->head!=NULL){ // prints the rest of the list
      cout<<temp->head->getKey();
      temp->head = temp->head->getNext();
    }
    cout<<endl;
  }
